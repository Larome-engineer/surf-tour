...
class TourRepository:
    ...
    async def get_all_tour_by_dest(self, destination_name) -> list[Tour]:
        return await (
            Tour.filter(tour_destination__destination=destination_name)
    ...
    async def get_future_paid_tour(self, name) -> Tour | None:
        ...
        if not tour:
            return None
        if tour.start_date < date.today() or not await TourPayment.filter(tour=tour).exists():
            return None
        return tour

    async def get_all_booked_tours(self) -> list[Tour]:
        return await (
            Tour.filter(user_tours__isnull=False)
            .distinct()
            .prefetch_related("tour_destination")
            .order_by("start_date")
        )
    ...
    async def get_upcoming_user_tours(self, tg_id) -> list[UserTour]:
        all_tours = await (
            UserTour.filter(user__user_tg_id=tg_id)
